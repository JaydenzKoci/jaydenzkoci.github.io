document.addEventListener('DOMContentLoaded', () => {
    ...
    const audioModule = {
      ...
      toggleMute: () => {
        ...
        if (!state.isMuted && state.currentPreviewUrl && elements.videoPopup.style.display !== 'block') {
          audio
            .play()
            .then(() => {
              console.log('Audio unmuted:', state.currentPreviewUrl);
              if (state.fadeInAudioEnabled) {
                audioModule.fadeInAudio(audio, 0.25, 2000);
              } else {
                audio.volume = 0.25;
              }
            })
    ...
    const modalModule = {
        ...
        renderModal: (track) => {
          const { title, artist, releaseYear, cover, duration, complete, difficulties, bpm, createdAt, lastFeatured, previewUrl, download, videoUrl, videoPosition, key, youtubeLinks, loading_phrase, videoZoom } = track;
          ...
          // Update or create loading phrase section
    ...
    const trackModule = {
      ...
      filterTracks: () => {
        ...
        filteredTracks.sort((a, b) => {
          if (filterValue === 'rotated') return new Date(b.lastFeatured) - new Date(a.lastFeatured);
          if (filterValue === 'new' || filterValue === 'finish') return new Date(b.createdAt) - new Date(a.createdAt);
          if (a.featured && !b.featured) return -1;
          if (!a.featured && b.featured) return 1;
          return new Date(b.createdAt) - new Date(a.createdAt);
        });
        state.currentFilteredTracks = filteredTracks;
        elements.trackCount.textContent = query || filterValue !== 'all' ? `Found: ${filteredTracks.length}` : `Total: ${state.tracksData.length}`;
    ...
    const countdownModule = {
      updateCountdown: () => {
        ...
        elements.countdown.textContent = `Last Updated - 04/20/25`;
      },
    };
